...
differences_p <- rasterToPoints(differences, spatial=T)
differences_df  <- data.frame(differences_p)

bird_occs <- ggplot() + theme_bw() + geom_raster(data=cloud_forest_df, aes(x=x, y=y, fill=MODCF_CloudForestPrediction)) + scale_fill_gradient(low="white", high="black") + geom_sf(data = study_region_crop, fill = NA)

test <- ggplot() + theme_bw() + geom_raster(data=differences_df, aes(x=x, y=y, fill=layer)) + scale_fill_gradient(low="white", high="black")

#Add scale
final_bird_occs <-test + theme(legend.position="none", panel.background = element_rect(fill = "aliceblue"))+ scalebar(x.min = -84, x.max = -81.7, y.min =-3, y.max = -5,dist = 100, st.dist=.1, st.size=1.9, height=.19, transform = TRUE, dist_unit = "km", model = 'WGS84') + north(study_region_crop, location="bottomleft", scale=.07, symbol=1) + ylab("Latitude") + xlab("Longitude") 
